...
router.post('/product/:id/stock_update_self_managed', async(request, response) => {
...
router.post('/product/:id/stock_update_managed', async(request, response) => {
	// Assuming Product ID and Product ID + 1 represent the same product type
	const data = request.body;
	try{
		const result = await sequalize.transaction(async (t) => {
			...
			// throw new Error();
			response.status(200).json({"message": "Updated both the products"});
		});
	} catch (e) {
		response.json(e);
	}
